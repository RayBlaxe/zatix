...
export default function EditTNCPage() {
  ...
  const [tncItem, setTncItem] = useState<TNCItem | null>(null)
  ...
  useEffect(() => {
    const fetchTNC = async () => {
      try {
        ...
        const allTnc = Array.isArray(response) ? response : response.data || []
        const foundTnc = allTnc.find((item: TNCItem) => item.id === parseInt(params.id as string))
        
        if (!foundTnc) {
          throw new Error("TNC not found")
        }

        setTncItem(foundTnc)
        form.setValue("content", foundTnc.content)
        form.setValue("type", foundTnc.type)
      } catch (err) {
        console.error("Failed to fetch TNC:", err)
        const errorMessage = err instanceof Error ? err.message : "Failed to fetch TNC"
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      await tncApi.updateTNC(token, tncItem.id, values)
      
      toast.success("TNC updated successfully!")
      router.push("/dashboard/tnc")
    } catch (err) {
      console.error("Failed to update TNC:", err)
      const errorMessage = err instanceof Error ? err.message : "Failed to update TNC"
      setError(errorMessage)
      toast.error(errorMessage)
    } finally {
    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-3xl font-bold tracking-tight">Edit TNC</h2>
        </div>
      </div>
